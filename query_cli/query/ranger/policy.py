...
from apache_ranger.model.ranger_policy import RangerPolicyItemAccess, RangerPolicy, RangerPolicyItem, RangerPolicyResource

from query.ranger import service2
...
class PolicyBuilder():
    ...
    def schema_resource(self, database_name: str, schema_name: str) -> 'PolicyBuilder':
        self._resources.append(
            { 
                'catalog': RangerPolicyResource({ 'values': ['delta'] }),
                'schema': RangerPolicyResource({ 'values': ["{}_{}".format(database_name, schema_name)] }),
                'table': RangerPolicyResource({ 'values': ['*'] }),
            }
        ) 
        return self  
    
    def all_tables_resource(self, database_name: str, schema_name: str) -> 'PolicyBuilder':
