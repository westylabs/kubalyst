...
from query.trino import service, client
...
def query(org_id: str, statement: str, session: Optional[client.Session] = None) -> QueryResponse:
    if session is None:
    ...
    while "nextUri" in response_body:
        ...
        if response_body.get("stats", {}).get("state") == "FAILED":
            raise ValueError("Query failed: {}".format(response_body["error"]["message"]))
